...
public sealed class PeopleRecognitionService
{
    ...
    private async Task<List<FaceEmbedding>> DetectAndStoreFacesAsync(string path, CancellationToken ct)
    {
        ...
        var faces = faceDetector.DetectFaces(image, DefaultMinScore);
        if (faces.Count == 0)
            return new List<FaceEmbedding>();
        ...
        for (var i = 0; i < faces.Count; i++)
        {
            ct.ThrowIfCancellationRequested();
            var face = faces[i];
            var embedding = faceRecognizer.ExtractEmbedding(image, face);
    ...
    {
        return await db.Db.Table<FaceEmbedding>()
